  ...
  <div class="top-up-main">
    ...
    <TransactionDetail title="收款回单" :trade-type="basicInfo.transactionType" :trade-time="basicInfo.transactionTime"
      ...
      <div class="current-content">
        <div class="title-style">充值企业 : {{basicInfo.enterpriseName}}</div>
        <div class="title-style">统一社会信用代码 : {{basicInfo.enterpriseCreditCode}}</div>
        <div class="title-style">
          充值金额 : {{basicInfo.amount | toThousands}}
...
export default {
  ...
  data () {
    return {
      ...
      // 充值详单 Url
      getDepositUrl: settings.apiGateway.getAccountDetail,
      // 检查是否交易成功
  ...
  methods: {
    checkOderStatus () {
      this.fetch(`${this.getDepositUrl}/${this.txCode}`).then(data => {
  ...
  beforeRouteLeave (to, from, next) {
    if (to.path !== '/enterprise/top-up' && to.path !== '/enterprise/cash-detail-account') {
      next(vm => {
