  ...
  <div class="buy-main">
    ...
    <TransactionDetail title="收款回单" :trade-type="basicInfo.transactionType" :trade-time="basicInfo.transactionTime"
      ...
      <div class="current-content">
        <div class="title-style">购买企业 : {{basicInfo.enterpriseName}}</div>
        <div class="title-style">统一社会信用代码 : {{basicInfo.enterpriseCreditCode}}</div>
        <div class="title-style">
          购买金额 : {{basicInfo.amount | toThousands}}
...
export default {
  ...
  data () {
    return {
      ...
      // 购买详单 Url
      getBuyUrl: settings.apiGateway.getAccountDetail
    }
  },
  created () {
  ...
  methods: {
    checkOderStatus () {
      this.fetch(`${this.getBuyUrl}/${this.txCode}`).then(data => {
