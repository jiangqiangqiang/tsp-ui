...
<template>
  <div>
    ...
    <div class="loadingText" v-else-if="status === 'Pending' || status === 'Sending'">
...
export default {
  ...
  methods: {
    goBack () {
      // 返回
      this.$router.replace('/enterprise/top-up')
    },
    getRecord () {
      this.$router.replace('/enterprise/top-up')
    },
    getTransactionDetails () {
      this.$router.replace({
        path: `/${this.role}/top-up/top-up-deal`,
        query: {
          code: this.txCode
        }
      })
    ...
    continueTopUp () {
      this.$router.replace('/enterprise/top-up/start-top-up')
    },
    checkOderStatus () {
      this.fetch(`${this.checkIsTopUpScueessUrl}${this.$route.query.code.split('?')[0]}`).then(data => {
        ...
        } else if (this.status === 'Pending' || this.status === 'Sending') {
          setTimeout(() => {
