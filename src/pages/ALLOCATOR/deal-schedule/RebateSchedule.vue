  ...
  <div class="top-up-main">
    ...
    <TransactionDetail title="收款回单" :trade-type="basicInfo.transactionType" :trade-time="basicInfo.transactionTime"
      ...
      <div class="current-content">
        <div class="title-style">企业名称 : {{basicInfo.relatedPartyName}}</div>
        <div class="title-style">统一社会信用代码 : {{basicInfo.relatedPartyCreditCode}}</div>
        <div class="title-style">
          实收金额 : {{basicInfo.amount | toThousands}}
...
export default {
  ...
  data () {
    return {
      ...
      // 返回详单 Url
      getRebateUrl: settings.apiGateway.getAccountDetail,
      checkIsTopUpScueessUrl: settings.apiGateway.getAccountDetail
    }
  },
  created () {
  ...
  methods: {
    checkOderStatus () {
      this.fetch(`${this.getRebateUrl}/${this.txCode}`).then(data => {
