<template>
  <div class="top-up-main">
    <BusinessTitle isShowBack title="金币明细账"></BusinessTitle>
    <TransactionDetail title="奖励入账回单" :trade-type="basicInfo.transactionType" :trade-time="basicInfo.transactionTime"
...
export default {
  ...
  data () {
    return {
      ...
      // 奖励金币详单 Url
      getAllocatorAwardUrl: settings.apiGateway.getAccountDetail
    }
  },
  created () {
  ...
  methods: {
    checkOderStatus () {
      this.fetch(`${this.getAllocatorAwardUrl}/${this.txCode}`).then(data => {
