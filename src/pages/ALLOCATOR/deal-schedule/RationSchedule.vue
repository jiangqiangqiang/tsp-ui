  ...
  <div class="top-up-main">
    ...
    <TransactionDetail title="收款回单" :trade-type="basicInfo.transactionType" :trade-time="basicInfo.transactionTime"
      ...
      <div class="current-content">
        <div class="title-style">关联方 : {{basicInfo.relatedPartyName}}</div>
...
export default {
  ...
  data () {
    return {
      ...
      // 返回详单 Url
      getRationUrl: settings.apiGateway.getAccountDetail,
  ...
  methods: {
    checkOderStatus () {
      this.fetch(`${this.getRationUrl}/${this.txCode}?tag_ep=benefit`).then(data => {
