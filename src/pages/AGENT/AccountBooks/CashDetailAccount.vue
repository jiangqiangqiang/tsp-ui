...
export default {
  ...
  methods: {
    ...
    query () {
      if (this.pageFlag) {
        this.pageNo = this.$store.state.searchCache.agentCashAccount.pageNo
        this.pageSize = this.$store.state.searchCache.agentCashAccount.pageSize
      } else {
        this.pageNo = 1
      }
      this.getCashAccountData()
  ...
  beforeRouteEnter (to, from, next) {
    if (from.path.indexOf('schedule') > -1) {
      next(vm => {
        vm.filtrate = vm.$store.state.searchCache.agentCashAccount
        vm.pageFlag = true
      })
    } else {
      next(vm => {
...
.cash-detail-main/deep/ .has-gutter th {
  padding-top: 3px;
  padding-bottom: 3px;
}
