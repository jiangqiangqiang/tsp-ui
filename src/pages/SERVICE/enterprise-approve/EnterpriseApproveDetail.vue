<template>
  <div class="enterprise-approve">
    ...
    <EnterpriseDetail title="企业信息审核" :Id=this.Id :sysRoles="$route.query.tag" @statusData='statusData'>
    ...
    <div>
      <el-dialog :visible.sync="logoutDialogShow" center width="570px" top="297px">
        <div class="text">是否确认通过&nbsp;<span class="color-change">{{name}}&nbsp;</span>的企业信息审核?</div>
    ...
    rejectSuccess () {
      ...
      this.fetch(this.enterpriseAuditUrl, 'post', data).then(data => {
        ...
        this.popUpSuccess({
          ...
          okBtn: '返回',
          ok: () => {
            this.$router.push('/service/enterprise-approve')
          }
        })
      }).catch(data => {
        this.$message({
